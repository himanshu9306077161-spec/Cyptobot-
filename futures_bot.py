...
def place_trade(symbol, direction, bal, score, momentum, details=None):
    ...
    if LIVE_TRADING:
        ...
        # ── STOP LOSS ─────────────────────────────────────────────
        # Correct Binance Futures format:
        # STOP_MARKET + closePosition:true (no quantity, no reduceOnly)
        sl_placed = False
        ...
        if sl_result and sl_result.get("orderId"):
            log(f"  ✅ Stop Loss @ ${sl_rounded} (order {sl_result['orderId']})")
        ...
        # ── TAKE PROFIT ────────────────────────────────────────────
        # Correct Binance Futures format:
        # TAKE_PROFIT_MARKET + closePosition:true (no quantity, no reduceOnly)
        tp_placed = False
        ...
        if tp_result and tp_result.get("orderId"):
            log(f"  ✅ Take Profit @ ${tp_rounded} (order {tp_result['orderId']})")

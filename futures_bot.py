...
import os, sys, json, time, math, hmac, hashlib, random, urllib3
...
def get_working_proxy():
    """Get a working free proxy to bypass Binance India geo-restriction."""
    sources = [
    ...
    print(f"  Found {len(proxies_list)} proxies to test...")
    random.shuffle(proxies_list)

    for proxy_str in proxies_list[:60]:
        # Try both http and https proxy formats
        for proxy_format in [
            {"http": f"http://{proxy_str}", "https": f"http://{proxy_str}"},
            {"http": f"http://{proxy_str}", "https": f"https://{proxy_str}"},
        ]:
            try:
                # Test with HTTPS signed-style endpoint
                r = requests.get(
                ...
                continue
    print("  ⚠ No proxy worked — using direct connection")
    return None

print("[PROXY] Finding working proxy for Binance...")

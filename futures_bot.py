...
def place_trade(symbol, direction, bal, score, momentum):
    ...
    # Notional = qty * price = position value on exchange
    # Binance minimum notional is $20 for all futures pairs
    notional = qty * px
    log(f"  Qty={qty} Price=${px} Notional=${notional:.2f}")

    if notional < 20:
        # Increase qty to meet $20 minimum
    ...
    # Final check — notional must be <= 95% of max position we can open
    max_notional = bal * 0.95 * leverage

...
def place_trade(symbol, direction, bal, score, momentum):
    ...
    # Check minimum notional value ($20 minimum on Binance Futures)
    notional = qty * px
    if notional < 20:
        # Increase qty to meet minimum $20 notional
        min_qty_needed = 20 / px
        steps_needed   = math.ceil(min_qty_needed / step)
        qty            = round(steps_needed * step, 8)
        notional       = qty * px
        log(f"  Adjusted qty to {qty} for min notional ${notional:.2f}")

    # If even with adjustment capital is too small skip this coin
    if notional > pos_val * 1.5:
        log(f"  ⚠ Skipping {symbol} — notional ${notional:.2f} too large for capital ${pos_val:.2f}")
        return None

    tp_pct     = calc_take_profit(score, momentum)
    init_stop  = INIT_STOP

    # Dynamic leverage based on signal + volatility
    atr_val    = details.get("atr", 2.0) if hasattr(details, "get") else 2.0
    ...
    log(f"  📊 Score={score}/8 Momentum={momentum:.1f}% Leverage={leverage}x")

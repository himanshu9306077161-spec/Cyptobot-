...
def analyse(symbol, fng):
    ...
    try:
        ...
        # Fear & Greed filter: in extreme fear (<25), need 7+ signals for LONG
        min_sig = MIN_SIGNALS
        if fng < 25 and bull >= MIN_SIGNALS:
            min_sig = 7
            if bull < 7:
                d["skip"] = f"F&G={fng} extreme fear — need 7 signals got {bull}"
                return "NEUTRAL", bull, d, atr_val, adx_val

        if bull >= min_sig and bull_trend:
...
def main():
    ...
    # Circuit breaker
    start = state.get("start_balance", bal)
    daily_loss_pct = state.get("daily_loss",0) / start if start > 0 else 0
    if daily_loss_pct <= -DAILY_LOSS_CAP:
        log(f"⛔ Circuit breaker: {daily_loss_pct*100:.1f}% daily loss")

...
MAX_POSITIONS  = 10         # Up to 10 simultaneous trades
MAX_LEVERAGE   = 10         # 10x leverage always
MIN_LEVERAGE   = 5          # Min 5x
MAX_LOSS_PCT   = 0.05       # Stop loss 5%
TP_FIXED       = 0.10       # Take profit 10%
SPOT_THRESHOLD = 1000.0     # Transfer 50% to spot when balance >= $1000
...
def analyse(symbol, fng):
    ...
    try:
        ...
        # Signal 12: Fear & Greed — TREND following
        d["fng"] = fng
        if fng<=15:
            # Extreme fear — strong downtrend — 2 bear signals
            bear+=2; d["s12"]=f"🔴🔴 Extreme fear x2 SHORT {fng}"
        elif fng<=25:
            # Fear — downtrend continuing — 1 bear signal
            bear+=1; d["s12"]=f"🔴 Fear SHORT {fng}"
        elif fng<=40:
            # Mild fear — neutral
            d["s12"]=f"⚪ Mild fear {fng}"
        elif fng>=85:
            # Extreme greed — topping — 2 bear signals
            bear+=2; d["s12"]=f"🔴🔴 Extreme greed x2 SHORT {fng}"
        elif fng>=70:
            # Greed — 1 bear signal
            bear+=1; d["s12"]=f"🔴 Greed SHORT {fng}"
        elif fng>=60:
            # Mild greed — neutral
            d["s12"]=f"⚪ Mild greed {fng}"
        else:
            # Neutral 40-60
        ...
        # Pick direction based on STRONGEST signal score
        # If both qualify, highest score wins
        bull_ok = bull >= MIN_SIGNALS and bull_trend
        bear_ok = bear >= MIN_SIGNALS and bear_trend

        if bull_ok and bear_ok:
            # Both qualify — pick strongest
            if bear > bull:
...
def calc_tp_sl(price, direction, atr_val, adx_val):
    """
    Fixed 10% TP and 5% SL for small positions.
    With 10x leverage:
      5% price move = 50% loss on capital ($0.50 on $1)
      10% price move = 100% gain on capital ($1.00 on $1)
    Risk/Reward = 2:1 in our favour.
    """
    sl_pct  = MAX_LOSS_PCT   # 5%
    tp1_pct = TP_FIXED        # 10%
    tp2_pct = TP_FIXED * 1.5  # 15% (let half run further)
...
# ── POSITION SIZING ───────────────────────────────────────────────
def calc_size(balance, price, sl_price, capital_each=1.0):
    """
    Dynamic position sizing using capital_each from get_trade_config().
    capital_each is determined by account balance tier.
    Always uses 10x leverage for maximum position size.
    """
    capital = min(capital_each, balance * 0.20)
    capital = max(capital, 0.50)

    leverage = MAX_LEVERAGE  # Always 10x

    actual_pos = capital * leverage

    # Ensure above Binance $20 minimum
    if actual_pos < 20:
        leverage = math.ceil(20.0 / capital)
...
# ── PLACE TRADE ───────────────────────────────────────────────────
def place_trade(symbol, direction, balance, score, atr_val, adx_val, capital_each=1.0):
    try:
        price = get_price(symbol)
        if not price: return None

        sl, tp1, tp2 = calc_tp_sl(price, direction, atr_val, adx_val)
        pos_usd, capital, leverage = calc_size(balance, price, sl, capital_each)
...
def get_trade_config(balance):
    ...
    Returns (num_trades, capital_per_trade)

    < $10:   Distribute 60% across available trades
    $10-19:  10 trades × $1
    $20-29:  10 trades × $2
    $30-49:  10 trades × $3
    $50-99:  10 trades × $5
    $100-199: 10 trades × $10
    $200-499: 10 trades × $20
    $500-999: 10 trades × $50
    $1000+:  Transfer 50% to spot, trade remaining 50%
    """
    if balance < 2:
        return 0, 0
    ...
    if balance < 10:
        ...
        # $1000+ — use 50% for trading (50% transferred to spot)
        trading_bal = balance * 0.50
        num, cap = 10, round(trading_bal / 10, 2)
...
def transfer_to_spot(amount):
    ...
    result = bpost("/sapi/v1/futures/transfer", {
        "asset": "USDT",
        "amount": str(round(amount, 2)),
        "type": "2",  # 2 = Futures → Spot
    })
    if result and result.get("tranId"):
        log(f"✅ Transferred ${amount:.2f} USDT to Spot wallet")
    else:
        log(f"⚠ Transfer failed: {result}")
...
def main():
    ...
    log(f"  Signals: 13 | Min: {MIN_SIGNALS} | TP: {TP_FIXED*100:.0f}% | SL: {MAX_LOSS_PCT*100:.0f}%")
    log(f"  Leverage: {MIN_LEVERAGE}-{MAX_LEVERAGE}x | TP: {TP_FIXED*100:.0f}% | SL: {MAX_LOSS_PCT*100:.0f}% | Max: {MAX_POSITIONS} trades")
    ...
    if max_trades == 0:
        log("⚠ Balance too low for any trades")
        save_state(state); return

    log(f"\n💼 Strategy: {max_trades} trades × ${capital_each:.2f} each "
        f"= ${max_trades*capital_each:.2f} total capital")

    # ── Auto transfer 50% to Spot if balance >= $1000 ─────────────
    if bal >= SPOT_THRESHOLD and LIVE_TRADING:
        transfer_amount = round(bal * 0.50, 2)
        log(f"\n💸 Balance ${bal:.2f} >= $1000!")
        log(f"  Transferring ${transfer_amount:.2f} to Spot wallet automatically")
        transfer_to_spot(transfer_amount)
        bal -= transfer_amount
        max_trades, capital_each = get_trade_config(bal)
        log(f"  Remaining for trading: ${bal:.2f}")

    # Max positions check
    if len(state["positions"]) >= max_trades:
        log(f"Max {max_trades} positions open — not opening new")
        save_state(state); return
    ...
    for sym, direction, score, details, atr_val, adx_val in signals[:slots]:
        ...
        record = place_trade(sym, direction, bal, score, atr_val, adx_val, capital_each)
        if record:

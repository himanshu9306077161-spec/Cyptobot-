...
MIN_SIGNALS   = 6           # Need 6/13
MAX_OPEN      = 5           # Max 5 positions
MAX_LOSS_PCT  = 0.12        # Hard cap 12% — protects small balance
...
def get_top_symbols():
    # SAFE COINS ONLY — proven liquidity, established projects
    # Excludes meme coins, new launches, high volatility unknown coins
    SAFE_COINS = [
        "BTCUSDT",   # Bitcoin — most liquid
        "ETHUSDT",   # Ethereum — second most liquid
        "BNBUSDT",   # BNB — exchange token
        "SOLUSDT",   # Solana — top L1
        "XRPUSDT",   # XRP — established
        "ADAUSDT",   # Cardano — established
        "AVAXUSDT",  # Avalanche — top L1
        "DOTUSDT",   # Polkadot — established
        "LINKUSDT",  # Chainlink — top oracle
        "LTCUSDT",   # Litecoin — established
        "ATOMUSDT",  # Cosmos — established
        "NEARUSDT",  # NEAR — top L1
        "APTUSDT",   # Aptos — top L1
        "ARBUSDT",   # Arbitrum — top L2
        "OPUSDT",    # Optimism — top L2
        "INJUSDT",   # Injective — established DeFi
        "SUIUSDT",   # Sui — top L1
        "SEIUSDT",   # Sei — top L1
        "TIAUSDT",   # Celestia — established
        "WLDUSDT",   # Worldcoin — established
    ]

    # Verify they are tradeable and have good volume
    d = bget("/fapi/v1/ticker/24hr")
    if not d:
        return SAFE_COINS

    vol_map = {t["symbol"]: float(t.get("quoteVolume",0)) for t in d}
    tradeable = [s for s in SAFE_COINS
                 if vol_map.get(s, 0) >= 5_000_000]  # $5M minimum for safe coins

    log(f"Trading {len(tradeable)} safe established coins")
    return tradeable if tradeable else SAFE_COINS
...
# ── CANDLESTICK PATTERNS (Signal 13) ─────────────────────────────
def candle_patterns(o, h, l, c, v):
    """
    Detect the 6 most reliable candlestick patterns.
    Research verified: 65-80% win rate with volume confirmation.
    Uses 1h candles only — most reliable timeframe.
    ...
    o1,h1,l1,c1,v1 = o[-1],h[-1],l[-1],c[-1],v[-1]  # current
    o2,h2,l2,c2,v2 = o[-2],h[-2],l[-2],c[-2],v[-2]  # previous
    o3,h3,l3,c3,v3 = o[-3],h[-3],l[-3],c[-3],v[-3]  # 2 back
    avg_vol = sum(v[-6:-1])/5 if len(v)>=6 else v1
    ...
    vol_confirm = v1 > avg_vol * 1.2  # 20% above average

    # ── BULLISH PATTERNS ─────────────────────────────────────────

    # 1. Hammer — long lower wick, small body at top, after downtrend
    # Lower wick >= 2x body, body in top 35% of candle
    downtrend = c2 < o2 and c3 < o3  # 2 prior bear candles
    hammer = (lower1 >= body1 * 2.0 and
              body1 <= range1 * 0.35 and
              upper1 <= body1 * 0.5 and
              downtrend)
    if hammer:
        return "BULL"  # Hammer — buyers rejected selloff

    # 2. Bullish Engulfing — big green swallows previous red
    # Current green body > previous red body, volume confirms
    bull_engulf = (bull_candle1 and bear_candle2 and
                   c1 > o2 and o1 < c2 and
                   body1 > body2 and
                   vol_confirm)
    if bull_engulf:
        return "BULL"  # Strongest bullish reversal signal

    # 3. Morning Star — 3 candle reversal (red, doji, green)
    body3 = abs(c3-o3)
    bear_candle3 = c3 < o3
    bull_candle_c1 = c1 > o1
    morning_star = (bear_candle3 and
                    body2 <= (h2-l2)*0.3 and   # middle = small body
                    bull_candle_c1 and
                    c1 > (o3+c3)/2 and          # closes above midpoint of candle 3
                    body3 > body2)              # first candle has real body
    if morning_star:
        return "BULL"  # 3-candle reversal confirmation

    # ── BEARISH PATTERNS ─────────────────────────────────────────

    # 4. Shooting Star — long upper wick, small body at bottom, after uptrend
    uptrend = c2 > o2 and c3 > o3   # 2 prior bull candles
    shooting_star = (upper1 >= body1 * 2.0 and
                     body1 <= range1 * 0.35 and
                     lower1 <= body1 * 0.5 and
                     uptrend)
    if shooting_star:
        return "BEAR"  # Shooting star — sellers rejected rally

    # 5. Bearish Engulfing — big red swallows previous green
    bear_engulf = (bear_candle1 and bull_candle2 and
                   o1 > c2 and c1 < o2 and
                   body1 > body2 and
                   vol_confirm)
    if bear_engulf:
        return "BEAR"  # Strongest bearish reversal signal

    # 6. Evening Star — 3 candle reversal (green, doji, red)
    bull_candle3 = c3 > o3
    evening_star = (bull_candle3 and
                    body2 <= (h2-l2)*0.3 and    # middle = small body
                    bear_candle1 and
                    c1 < (o3+c3)/2 and           # closes below midpoint of candle 3
                    body3 > body2)
    if evening_star:
        return "BEAR"  # 3-candle reversal confirmation
...
def analyse(symbol, fng):
    ...
    # Define d FIRST before any early returns
    d = {"price": price, "atr": round(atr_val,6),
         "adx": round(adx_val,1), "regime": regime}

    # Skip choppy markets immediately
    if regime == "choppy":
        return "NEUTRAL", 0, {**d, "skip": f"ADX={adx_val:.1f} choppy"}, atr_val, adx_val

    # Skip extremely volatile coins (ATR > 5% = dangerous for small account)
    atr_pct = (atr_val / price) * 100
    if atr_pct > 5.0:
        return "NEUTRAL", 0, {**d, "skip": f"ATR={atr_pct:.1f}% too volatile"}, atr_val, adx_val

    # Trend confirmation — last 3 candles must confirm direction
    last3_bull = c[-1]>c[-2] and c[-2]>c[-3] and c[-3]>c[-4]
    last3_bear = c[-1]<c[-2] and c[-2]<c[-3] and c[-3]<c[-4]
    d["trend_confirmed_bull"] = last3_bull
    d["trend_confirmed_bear"] = last3_bear

    bull = bear = 0
    # d already defined above — just reset bull/bear counters
    ...
    candle_sig = candle_patterns(
        k1h["o"], k1h["h"], k1h["l"], k1h["c"], k1h["v"]
    )
    d["candle"] = candle_sig
    if candle_sig == "BULL":
        bull += 1; d["s13"] = "🟢 Bullish candle pattern"
    elif candle_sig == "BEAR":
        bear += 1; d["s13"] = "🔴 Bearish candle pattern"
    else:
        d["s13"] = "⚪ No candle pattern"

    d["bull"]=bull; d["bear"]=bear

    # Require trend confirmation for entry
    # Last 3 candles must agree with signal direction
    last3_bull = d.get("trend_confirmed_bull", False)
    last3_bear = d.get("trend_confirmed_bear", False)

    if bull>=MIN_SIGNALS:
        if last3_bull:
            return "LONG", bull, d, atr_val, adx_val
        else:
            log(f"  {symbol}: LONG signal but trend not confirmed — skipping")
            return "NEUTRAL", bull, d, atr_val, adx_val

    if bear>=MIN_SIGNALS:
        if last3_bear:
            return "SHORT", bear, d, atr_val, adx_val
        else:
            log(f"  {symbol}: SHORT signal but trend not confirmed — skipping")
            return "NEUTRAL", bear, d, atr_val, adx_val
...
def place_trade(symbol, direction, balance, score, atr_val, adx_val, details):
    try:
        ...
        if LIVE_TRADING:
            # ALWAYS force ISOLATED margin — protects rest of balance
            # Research: 95% of retail traders should use isolated margin
            # Cross margin can drain entire account on one bad trade
            bpost("/fapi/v1/marginType", {
...
# ── MONITOR POSITIONS ─────────────────────────────────────────────
def check_signal_reversal(symbol, original_direction):
    """
    Check if signals have reversed against our position.
    Used for smart position replacement after 2 hours.
    Returns True if signals now strongly oppose our position.
    """
    try:
        k = klines(symbol, "1h", 100)
        if not k: return False
        c = k["c"]; h = k["h"]; l = k["l"]

        # Quick signal check — just 4 key indicators
        r = rsi(c)
        e20 = ema(c, 20); e50 = ema(c, 50)
        up, mid, lo = bollinger(c)
        bb_pct = (c[-1]-lo)/(up-lo) if up!=lo else 0.5
        m, s, hist = macd(c)
        atr_v = atr(h, l, c)
        adx_v = adx(h, l, c)
        ...
        if original_direction == "LONG":
            if r > 70:          opposing += 1  # overbought
            if e20 < e50:       opposing += 1  # EMA bearish
            if bb_pct > 0.85:   opposing += 1  # near upper band
            if m < s:           opposing += 1  # MACD bearish
        else:
            if r < 30:          opposing += 1  # oversold
            if e20 > e50:       opposing += 1  # EMA bullish
            if bb_pct < 0.15:   opposing += 1  # near lower band
            if m > s:           opposing += 1  # MACD bullish

        # 3 or more opposing signals = reversal confirmed
        return opposing >= 3
    except:
        return False
...
def monitor(state):
    ...
    for pos in state["positions"]:
        ...
        # ── SMART REPLACEMENT CHECK ────────────────────────────────
        # After 2 hours: if position is losing AND signals reversed
        # → Close early to free capital for better opportunity
        hours_open = (time.time()-pos.get("entry_time",time.time())) / 3600
        if not reason and hours_open >= 2 and pnl_pct < -0.03:
            # Position has been open 2+ hours and is losing 3%+
            signals_reversed = check_signal_reversal(sym, dir)
            if signals_reversed:
                reason = f"🔄 Smart replace: signals reversed {pnl_pct*100:.1f}%"
                log(f"  ⚡ {sym}: signals reversed after {hours_open:.1f}h — replacing position")
                pos["replace"] = True  # Flag for replacement
...
def main():
    log("="*64)
    log("  VIRALAB CRYPTO FUTURES BOT v4.1")
    ...
    # Check for replaced positions — these freed up capital
    replaced = [p for p in state.get("closed_trades",[])
                if p.get("replace") and
                (time.time() - p.get("entry_time",0)) < 3600]
    if replaced:
        log(f"\n🔄 {len(replaced)} position(s) replaced — will find better opportunities")

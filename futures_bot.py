...
def calc_take_profit(score, momentum):
    ...
    Stronger signals + higher momentum = higher profit target.
    """
    # Base profit from signal score (5-8 signals)
    base = {5: 0.15, 6: 0.30, 7: 0.50, 8: 0.80}.get(score, 0.15)

    # Momentum multiplier (0-5% momentum = 1x, 5-10% = 1.5x, 10%+ = 2x)
    if momentum > 10:   mult = 2.0
    elif momentum > 5:  mult = 1.5
    else:               mult = 1.0

    tp = base * mult
    tp = max(0.12, min(tp, 1.50))  # Between 12% and 150%
    return round(tp, 2)
...
def place_trade(symbol, direction, bal, score, momentum):
    px = None
    ticker = bget("/fapi/v1/ticker/price", {"symbol": symbol})
    if ticker: px = float(ticker["price"])
    if not px: return None

    size_pct   = kelly_size({}, bal, score)  # Will use real state in monitor
    ...
    log(f"  📊 Score={score}/8 Momentum={momentum:.1f}%")
    ...
    if LIVE_TRADING:
        ...
        if sl_order:
            log(f"  ✅ Stop Loss set @ ${stop_price:,.4f} (-{INIT_STOP*100:.0f}%)")
        else:
            log(f"  ⚠ SL order failed — monitoring manually")

        # Place Take Profit
        tp_order = bpost("/fapi/v1/order",{
            "symbol":      symbol,
            "side":        sl_side,
        ...
        if tp_order:
            log(f"  ✅ Take Profit set @ ${tp_price:,.4f} (+{tp_pct*100:.0f}%)")
        else:
            log(f"  ⚠ TP order failed — monitoring manually")
...
def main():
    ...
    for sym, direction, score, details, momentum in signals[:slots]:
        ...
        record = place_trade(sym, direction, bal, score, momentum)
        if record:

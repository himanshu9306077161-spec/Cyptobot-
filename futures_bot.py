...
def place_trade(symbol, direction, bal, score, momentum, details=None):
    ...
    if LIVE_TRADING:
        ...
        # ── STOP LOSS ─────────────────────────────────────────────
        # Binance changed API on 2025-12-09:
        # STOP_MARKET must now use /fapi/v1/algoOrder NOT /fapi/v1/order
        sl_placed = False
        ...
        if sl_result and (sl_result.get("algoId") or sl_result.get("orderId")):
            ...
            log(f"  ✅ Stop Loss @ ${sl_rounded} (algoId {algo_id})")
            record["sl_order_id"] = algo_id
            sl_placed = True
        else:
        ...
        # ── TAKE PROFIT ────────────────────────────────────────────
        # Same: TAKE_PROFIT_MARKET must use /fapi/v1/algoOrder
        tp_placed = False
        tp_result = bpost("/fapi/v1/algoOrder", {
            ...
            "algoType":     "CONDITIONAL",
            "quantity":     str(qty),
            "triggerPrice": str(tp_rounded),
            "reduceOnly":   "true",
            "workingType":  "CONTRACT_PRICE",
        })
        if tp_result and (tp_result.get("algoId") or tp_result.get("orderId")):
            algo_id = tp_result.get("algoId") or tp_result.get("orderId")
            log(f"  ✅ Take Profit @ ${tp_rounded} (algoId {algo_id})")
            record["tp_order_id"] = algo_id
            tp_placed = True
        else:
            log(f"  ❌ TP failed: {tp_result}")

        log(f"  {'✅' if sl_placed and tp_placed else '⚠'} SL={sl_placed} TP={tp_placed}")
